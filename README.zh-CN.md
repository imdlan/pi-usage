...
当前支持 **Z.ai / GLM Coding Plan**；架构上可扩展更多供应商，无需改动核心代码。
...
需要 Node.js 20+，以及在 Pi 中配置的 Z.ai 供应商（base URL 指向官方 Z.ai / GLM 端点，API key 解析到你的 GLM Coding Plan token）。
...
添加供应商：在 `src/providers/<name>.ts` 实现 `UsageProvider`（认证策略、白名单、脱敏规则），在 `registry.ts` 注册，添加测试（401/403/429/5xx、超时、白名单、脱敏）。
